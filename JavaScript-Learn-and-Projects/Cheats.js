...
let names = ["A", "B", "C"];
names.pop();                  // using pop() you can remove last index of the array
names.push("D");               // using push() you can add last index of the array
names.unshift("AA");            // using unshift() you can add starting index of the array
names.shift();                  // using shift() you can remove staing index od the array
names.pop();
names.push("C");
console.log(names);
...
let results = result.filter((nums) => nums >= 6);
console.log(results);
...
let tasks = ["gym", "study", "code", "sleep"];

let upperCase = tasks.map((num) => num.toUpperCase());
...
let user = isLoggedIn && hasSubcription ? "Access Granted" : "Access Denied";
console.log(user);
...
let plant = (fruits) => `Now 15 Year Plant has grow and giving ${fruits * 2} Mangoes`;
console.log(plant(100));
...
fruits.forEach(fruits=> console.log(`Fruits : ${fruits}`));


let phoneModel = {model : "Iphone 13", Company : "Apple", Price : 59000};
...
let highValueProducts = products.filter(p => p.price >= 5000);
console.log(highValueProducts);



const employees = [
    { name: "Brijesh", dept: "IT", salary: 60000 },
    { name: "Aman", dept: "HR", salary: 45000 },
    { name: "Sana", dept: "IT", salary: 75000 },
    { name: "Rahul", dept: "Sales", salary: 30000 }
];

let itEmployees = employees.filter(emp => emp.dept === "IT");

let itWithBonus = itEmployees.map( emp => {
    return { ...emp , salary: emp.salary + ( emp.salary * 0.1)};
});

console.log(itWithBonus);
